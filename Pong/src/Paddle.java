import java.awt.Color;

public class Paddle extends Sprite {

	private static final int PADDLE_HEIGHT = 100;
	private static final int PADDLE_WIDTH = 10;
	private static final Color PADDLE_COLOUR = Color.YELLOW;
	private static final int PADDLE_DISTANCE_FROM_EDGE = 40;
	
	public Paddle(Player player, int panelWidth, int panelHeight) {
		setWidth(PADDLE_WIDTH);
		setHeight(PADDLE_HEIGHT);
		setColour(PADDLE_COLOUR);
		
		if (player == Player.One) {
			setInitialPosition(PADDLE_DISTANCE_FROM_EDGE, panelHeight / 2 - (getHeight() / 2));	
		} else if (player == Player.Two) {
			setInitialPosition(panelWidth - PADDLE_DISTANCE_FROM_EDGE - getWidth(), panelHeight / 2 - (getHeight() / 2));	
		}

		/*
		 *           int xPos;
          if(player == Player.One) {
              xPos = DISTANCE_FROM_EDGE;
          } else {
              xPos = panelWidth - DISTANCE_FROM_EDGE - getWidth();
          }

          setInitialPosition(xPos, panelHeight / 2 - (getHeight() / 2));
		 */
		
		resetToInitialPosition();
	}
	
}
